# -*- coding: utf-8 -*-
"""CPGAPPO forward-only 消融: CP-aware guide + shield + app 连坐奖励, 前向 GAT only?
唯一改动：使?compute_guide_scores_cp 替代 compute_guide_scores?
"""
...
def eval_cpgappo_once(agent, ts, gs, bc, arrival_plan, task_complex_index, MAX_STEPS):
    ...
    print(f"[CPGAPPO_fwdonly][EVAL] AppTO={rho_app:.2%}, TaskTO={rho_task:.2%}, Score={score:.3f}, "
          f"E={e:.4f}, D={d:.4f}")
    print(f"[CPGAPPO_fwdonly][EVAL] action_stats: Local={partition['local']}, Cloud={partition['cloud']}, "
          f"Edge={partition['edge']}, Timeout={partition['timeout']}, Unknown={partition['unknown']}")
    print(f"[CPGAPPO_fwdonly][EVAL] TotalEnergy={total_energy:.4f}, Subtasks: {finished_sub}/{total_sub}")
    sys.stdout.flush()
    return e, d, score, to_info, total_energy
def train_cpgappo_fwdonly(gpu_id, seed_offset, episodes=20, lr=3e-4, entropy_coef=0.01, lambda_guide=0.2):
    """V3-guided + CP guide + shield + app credit training entry (fwdonly: use_backward=False)."""
    ...
    try:
        ...
        print(f"[CPGAPPO_fwdonly] Auto-detect: node_dim={state_dim}, global_dim={global_dim}")
    except Exception as e:
        print(f"[CPGAPPO_fwdonly] Dim detect failed, using defaults. Error: {e}")
        state_dim = 27
        global_dim = 10
    ...
    agent = GAT_PPO_Agent_CPGAPPO(
        ...
        use_backward=True,   # 双向 GAT (与基准一?
    )
    best_eval_score = float('inf')
    best_eval_appTO = 1.0
    best_eval_metrics = None
    print("[CPGAPPO_fwdonly] === Init eval ===")
    ...
    print(f"\n[CPGAPPO_fwdonly] === Training {episodes} episodes (lambda_guide={lambda_guide}) ===\n")
    for episode in range(episodes):
        ...
        episode_rewards = []
        # === 新增统计变量 ===
        shield_total = 0       # shield 被调用的次数（即有候选动作时?
        shield_blocked = 0     # shield 拦截/修改动作的次?
        app_fin_count = 0      # new_app_fin 触发次数
        app_to_count = 0       # new_app_to 触发次数
        ...
        # === 输出完整统计 ===
        print(f"[CPGAPPO_fwdonly] Ep {episode+1}/{episodes} train: AppTO={temp_app_to:.2%}, "
              f"Reward sum={ep_reward_sum:.2f}, mean={ep_mean:.4f}, std={ep_std:.4f}, max={ep_max:.4f}, min={ep_min:.4f}")
        print(f"[CPGAPPO_fwdonly] Ep {episode+1} stats: app_fin={app_fin_count}, app_to={app_to_count}, "
              f"shield_block={shield_blocked}/{shield_total}({shield_rate:.1%})")
        sys.stdout.flush()
        if curves_dir is not None:
            try:
                train_curve_file = curves_dir / f"CPGAPPO_fwdonly_seed{seed_offset}_train.csv"
                ...
                print(f"[CPGAPPO_fwdonly] 保存训练曲线失败: {curve_err}")
        if (episode + 1) % 5 == 0 or (episode + 1) == episodes:
            print(f"\n[CPGAPPO_fwdonly] --- Eval at Ep {episode+1} ---")
            ...
            if sc < best_eval_score:
                ...
                print(f"[CPGAPPO_fwdonly][KEEP] New best score={sc:.3f}, AppTO={eval_appTO:.2%}")
                if checkpoint_dir is not None:
                    try:
                        torch.save(agent.policy.state_dict(), checkpoint_dir / "CPGAPPO_fwdonly.pt")
                    except Exception as ckpt_err:
                        print(f"[CPGAPPO_fwdonly] 保存checkpoint失败: {ckpt_err}")
            else:
                print(f"[CPGAPPO_fwdonly][Monitor] Ep {episode+1}, AppTO={eval_appTO:.2%}, "
                      f"best_AppTO_so_far={best_eval_appTO:.2%}")
            sys.stdout.flush()
            if curves_dir is not None:
                try:
                    eval_curve_file = curves_dir / f"CPGAPPO_fwdonly_seed{seed_offset}_eval.csv"
                    ...
                    print(f"[CPGAPPO_fwdonly] 保存评估曲线失败: {eval_curve_err}")
    ...
    # 【可复现性修复】无?best 是否被刷新，都强制保?_last ckpt
    # 避免出现"训练成功?init eval score 从未被超??best ckpt 从不落盘"的情?
    if checkpoint_dir is not None:
        try:
            torch.save(agent.policy.state_dict(), checkpoint_dir / "CPGAPPO_fwdonly_last.pt")
        except Exception as ckpt_err:
            print(f"[CPGAPPO_fwdonly] 保存_last checkpoint失败: {ckpt_err}")
    print(f"\n[CPGAPPO_fwdonly][FINAL] Training complete!")
    print(f"[CPGAPPO_fwdonly][FINAL] Best eval AppTO: {best_eval_metrics['timeout_info']['app_timeout_rate']:.2%}")
    print(f"[CPGAPPO_fwdonly][FINAL] Best eval Score: {best_eval_metrics['score']:.3f}")
    sys.stdout.flush()
...
def train_forward_only(gpu_id, seed_offset, use_heuristic_sort=True, episodes=20, lr=3e-4, entropy_coef=0.01, lambda_guide=0.2):
    """?dividelong.py 统一调用的包装器"""
    e, d, metrics = train_cpgappo_fwdonly(
        gpu_id=gpu_id,
