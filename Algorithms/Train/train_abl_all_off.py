# -*- coding: utf-8 -*-
"""统一消融训练入口：通过5个开关控?CPGAPPO 各机制?
all_off = 全部关闭；nocp = 修复后真正关闭CP；其他变体也可通过此入口统一管理?
"""
...
from Algorithms.RealGATPPO.agent_cpgappo import (
    GAT_PPO_Agent_CPGAPPO, compute_guide_scores_cp, compute_guide_scores
)
from Algorithms.RealGATPPO.cpgappo_core import extract_cpgappo_state, compute_cpgappo_slack_reward
def build_safe_mask(ts, uid, now, mask_bin, guide_scores):
    """构建 safe_mask（与 Full 版本完全一致）"""
    ...
    if not safe_mask.any():
        safe_mask = np.array(mask_bin, dtype=bool)
    return safe_mask
def eval_once(agent, ts, gs, bc, arrival_plan, task_complex_index, MAX_STEPS,
    ...
    total_actions = 0
    guide_fn = compute_guide_scores_cp if use_cp else compute_guide_scores
    for slot in range(MAX_STEPS):
        ...
        for task in tasks:
            ...
            if use_cp or use_shield:
                guide_scores, guide_valid = guide_fn(ts, uid, task, task_complex_index, now)
                masked_scores = np.where(guide_valid, guide_scores, 1e6)
            else:
                masked_scores = None
            if use_shield and masked_scores is not None:
                action_mask = build_safe_mask(ts, uid, now, mask_bin, masked_scores)
            else:
...
def train_unified(gpu_id, seed_offset, episodes=20, lr=3e-4, entropy_coef=0.01,
                  lambda_guide=0.2, use_backward=True, use_shield=True,
                  use_app_credit=True, use_cp=True, algo_tag="unified"):
    """
    统一消融训练入口?
    参数:
        lambda_guide: Guide CE 权重 (0.0=关闭)
        use_backward: 是否使用 Backward GAT
        use_shield: 是否使用 Safety Shield
        use_app_credit: 是否使用 App Credit bonus
        use_cp: 是否使用 CP-aware guide scores
        algo_tag: 算法标识名（用于日志和曲线文件名?
    """
    ...
    print(f"[{algo_tag}] Config: guide={lambda_guide}, backward={use_backward}, "
          f"shield={use_shield}, credit={use_app_credit}, cp={use_cp}")
    print(f"[{algo_tag}] === Init eval ===")
    ee0, dd0, sc0, to0, te0 = eval_once(
        ...
        for slot in range(MAX_STEPS):
            ...
            for task in tasks:
                ...
                app_buffer.setdefault(uid, []).append([
                    ...
                    log_prob,
                    False,
                    action_mask if isinstance(action_mask, np.ndarray) else np.array(action_mask, dtype=bool),
                    guide_best_action
                ])
...
# ============ 各变体入口函数（?dividelong.py 调用?===========
def train_all_off(gpu_id, seed_offset, use_heuristic_sort=True, episodes=20, lr=3e-4, entropy_coef=0.01, lambda_guide=0.0):
    """all_off: 关闭全部5项机?(GAT 方向保留双向, ?CPGAPPO 一?"""
    e, d, metrics = train_unified(
        gpu_id=gpu_id, seed_offset=seed_offset, episodes=episodes,
        lr=lr, entropy_coef=entropy_coef,
        lambda_guide=0.0,        # 1. Guide CE 关闭
        use_backward=True,       # 5. 保留双向 GAT (?CPGAPPO 一?, ?forward-only ckpt 走兼容加载器
        use_shield=False,        # 2. Shield 关闭
        use_app_credit=False,    # 3. App Credit 关闭
        use_cp=False,            # 4. CP 排序关闭
        algo_tag="CPGAPPO_alloff",
    )
    return float(e), float(d), metrics
def train_wo_cpseq_fixed(gpu_id, seed_offset, use_heuristic_sort=True, episodes=20, lr=3e-4, entropy_coef=0.01, lambda_guide=0.2):
    """wo_cpseq (修复?: 真正关闭 CP，使?compute_guide_scores 替代 compute_guide_scores_cp"""
    e, d, metrics = train_unified(
        gpu_id=gpu_id, seed_offset=seed_offset, episodes=episodes,
        lr=lr, entropy_coef=entropy_coef,
