...
def eval_cpgappo_noshield_once(agent, ts, gs, bc, arrival_plan, task_complex_index, MAX_STEPS):
    ...
    for slot in range(MAX_STEPS):
        ...
        for task in tasks:
            ...
            action, _ = agent.take_action(
                s_data, g_feat, action_mask=mask_bin, deterministic=True
            )
            bc.step([[task, action]])
    ...
    print(f"[CPGAPPO_noshield][EVAL] AppTO={rho_app:.2%}, TaskTO={rho_task:.2%}, Score={score:.3f}, "
          f"E={e:.4f}, D={d:.4f}")
    print(f"[CPGAPPO_noshield][EVAL] action_stats: Local={partition['local']}, Cloud={partition['cloud']}, "
          f"Edge={partition['edge']}, Timeout={partition['timeout']}, Unknown={partition['unknown']}")
    print(f"[CPGAPPO_noshield][EVAL] TotalEnergy={total_energy:.4f}, Subtasks: {finished_sub}/{total_sub}")
    sys.stdout.flush()

    return e, d, score, to_info, total_energy


def train_cpgappo_noshield(gpu_id, seed_offset, episodes=20, lr=3e-4, entropy_coef=0.01, lambda_guide=0.2):
    ...
    try:
        ...
        print(f"[CPGAPPO_noshield] Auto-detect: node_dim={state_dim}, global_dim={global_dim}")
    except Exception as e:
        print(f"[CPGAPPO_noshield] Dim detect failed, using defaults. Error: {e}")
        state_dim = 27
        global_dim = 10
    ...
    print("[CPGAPPO_noshield] === Init eval ===")
    ee0, dd0, sc0, to0, te0 = eval_cpgappo_noshield_once(
        agent, ts, gs, bc, arrival_plan, task_complex_index, MAX_STEPS)
    best_eval_score = sc0
    best_eval_appTO = to0['app_timeout_rate']

    print(f"\n[CPGAPPO_noshield] === Training {episodes} episodes (lambda_guide={lambda_guide}) ===\n")
    for episode in range(episodes):
        ...
        for slot in range(MAX_STEPS):
            ...
            for task in tasks:
                ...
                action, log_prob = agent.take_action(s_data, g_feat, action_mask=mask_bin)
        ...
        print(f"[CPGAPPO_noshield] Ep {episode+1}/{episodes} train: AppTO={temp_app_to:.2%}, "
              f"Reward sum={ep_reward_sum:.2f}, mean={ep_mean:.4f}")
        sys.stdout.flush()

        if curves_dir is not None:
            try:
                train_curve_file = curves_dir / f"CPGAPPO_noshield_seed{seed_offset}_train.csv"
                ...
                print(f"[CPGAPPO_noshield] 保存训练曲线失败: {curve_err}")

        if (episode + 1) % 5 == 0 or (episode + 1) == episodes:
            print(f"\n[CPGAPPO_noshield] --- Eval at Ep {episode+1} ---")
            ee, dd, sc, to_info, te = eval_cpgappo_noshield_once(
                agent, ts, gs, bc, arrival_plan, task_complex_index, MAX_STEPS)
            ...
            if sc < best_eval_score:
                ...
                print(f"[CPGAPPO_noshield][KEEP] New best score={sc:.3f}, AppTO={eval_appTO:.2%}")
                if checkpoint_dir is not None:
                    try:
                        torch.save(agent.policy.state_dict(), checkpoint_dir / "CPGAPPO_noshield.pt")
                    except Exception as ckpt_err:
                        print(f"[CPGAPPO_noshield] 保存checkpoint失败: {ckpt_err}")
            else:
                print(f"[CPGAPPO_noshield][Monitor] Ep {episode+1}, AppTO={eval_appTO:.2%}, "
                      f"best_AppTO_so_far={best_eval_appTO:.2%}")
            sys.stdout.flush()

            if curves_dir is not None:
                try:
                    eval_curve_file = curves_dir / f"CPGAPPO_noshield_seed{seed_offset}_eval.csv"
                    ...
                    print(f"[CPGAPPO_noshield] 保存评估曲线失败: {eval_curve_err}")
    ...
    if checkpoint_dir is not None:
        try:
            torch.save(agent.policy.state_dict(), checkpoint_dir / "CPGAPPO_noshield_last.pt")
        except Exception as ckpt_err:
            print(f"[CPGAPPO_noshield] 保存_last checkpoint失败: {ckpt_err}")

    print(f"\n[CPGAPPO_noshield][FINAL] Training complete!")
    print(f"[CPGAPPO_noshield][FINAL] Best eval AppTO: {best_eval_metrics['timeout_info']['app_timeout_rate']:.2%}")
    print(f"[CPGAPPO_noshield][FINAL] Best eval Score: {best_eval_metrics['score']:.3f}")
    sys.stdout.flush()
...
def train_wo_shield(gpu_id, seed_offset, use_heuristic_sort=True, episodes=20, lr=3e-4, entropy_coef=0.01, lambda_guide=0.2):
    """供 dividelong.py 统一调用的包装器"""
    e, d, metrics = train_cpgappo_noshield(
        gpu_id=gpu_id,
